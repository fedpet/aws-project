...
Quando si crea un utente, la password viene criptata e salvata su DB. Quando viene richiesto l'autenticazione
e i parametri (username e password) corrispondono ai parametri salvati su DB, viene restituito un token che permette l'accesso alla piattaforma:
\begin{lstlisting}
...
\subsection{Frontend}
\subsubsection{Autentication}
Viene fatto una richiesta al backend passado username e password. Quando si riceve il token dal backend si salva quest'ultimo
in localStorage.
\begin{lstlisting}
